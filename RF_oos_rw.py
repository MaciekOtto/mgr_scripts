...
Wejście:  dane1000stopy.xlsx          (stopy zwrotu)
          garch_prognozy_oos.parquet  (prognozy GJR-GARCH jako feature)
...
Metodologia — identyczna jak GARCH:
  ...
  - Features: lag r² (1-5), lag zwrotów (1-5), rolling std 20d, σ²_GJR_t
 
Optymalizacja czasu (~3-5h na 4 rdzeniach, 3 workery):
  - RF retrenowany co RETRAIN_EVERY=10 dni zamiast co 1 dzień (~25x szybciej)
  - Małe drzewa: n_estimators=100, max_depth=10
  - Multiprocessing po spółkach (nie po dniach)
...
GARCH_FILE       = 'garch_prognozy_oos.parquet'
...
def build_features(returns_arr, gjr_arr=None):
    """
    ...
    use_gjr = gjr_arr is not None and len(gjr_arr) >= n
    ...
    for t in range(start, n - 1):
        ...
        if use_gjr:
            row['gjr_var'] = float(gjr_arr[t]) if not np.isnan(gjr_arr[t]) else np.nan
...
def process_ticker(ticker, returns_arr, gjr_arr, train_size, retrain_every, rf_params):
    X, y, t_idx = build_features(returns_arr, gjr_arr)
...
def process_batch(args):
    (batch_idx, tickers_batch, data_dict, gjr_dict,
...
def load_data():
    ...
    # GJR prognozy — z okna testowego (indeksy TRAIN_SIZE:)
    gjr_dict = {}
    if os.path.exists(GARCH_FILE):
        print(f"Wczytuję GJR-GARCH z: {GARCH_FILE}")
        df_g = pd.read_parquet(GARCH_FILE)
        if isinstance(df_g.columns, pd.MultiIndex):
            lvl0 = df_g.columns.get_level_values(0)
            if 'GJR-GARCH' in lvl0:
                gjr_df = df_g['GJR-GARCH']
                # GJR prognozy dotyczą tylko okna testowego (250 dni)
                # Dopasowujemy do pełnego szeregu: padding NaN z przodu
                for ticker in gjr_df.columns:
                    full = np.full(len(df), np.nan)
                    vals = gjr_df[ticker].values
                    full[TRAIN_SIZE:TRAIN_SIZE + len(vals)] = vals
                    gjr_dict[ticker] = full
    else:
        print("Brak garch_prognozy_oos.parquet — pomijam feature GJR-GARCH")
...
if __name__ == '__main__':
    ...
    batch_args = [
        (i, batch, data_dict, gjr_dict, TRAIN_SIZE,
